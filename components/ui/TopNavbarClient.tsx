...
const TopNavbarClient = ({ businessName }: Props) => {
  const pathname = usePathname()

  // En el catalogo publico (/order), el boton de "Inicio" se traslada al menu
  // lateral de categorias/filtros en movil para dejar mas espacio al catalogo.
  // En desktop (md+) esta barra se mantiene visible siempre.
  const hideOnMobile = pathname?.startsWith("/order") ?? false

  return (
    <header
      className={`sticky top-0 z-50 border-b border-slate-200/80 bg-white/90 backdrop-blur ${hideOnMobile ? "hidden md:block" : ""}`}
    >
      <nav
